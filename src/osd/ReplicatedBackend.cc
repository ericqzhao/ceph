  ...
  string omap_recovered_to)
{
  ...
  pgt->safe_create_traverse(
	...
	t->omap_setheader(coll, goid, *(op.omap_header));
      ...
      // if str1 == str2, return true
      auto compareStrings = [] (string& str1, const string& str2) {
      ...
      // compare with omap_recovered_to
      // and decide if we need to send omap op.
	...
	case UpdateType::Remove:
          {
            set<string> to_rm;
            bufferlist::const_iterator pt = up.second.begin();
            decode(to_rm, pt);
            for (set<string>::iterator i = to_rm.begin(); i != to_rm.end();) {
              auto should_send_kv_op = compareStrings(omap_recovered_to, *i);
              if (!should_send_kv_op) {
                to_rm.erase(i++); // not to send
              } else {
                i++;
              }
            }
            if (to_rm.size()) {
              t->omap_rmkeys(coll, goid, to_rm);
            }
          }
          break;
	case UpdateType::Insert:
          {
            ...
            bufferlist::const_iterator pt = up.second.begin();
            decode(to_set, pt);
            for (map<string, bufferlist>::iterator i = to_set.begin(); i != to_set.end();) {
              auto should_send_kv_op = compareStrings(omap_recovered_to, i->first);
              if (!should_send_kv_op) {
                to_set.erase(i++); // not to send
              } else {
                i++;
              }
            }
      ...
      if (parent->get_backfill_shards().size() >= 1) {
        for (const auto& shard : parent->get_backfill_shards()) {
          ...
          generate_transaction_for_backfilling_write(t, coll, log_entries,
            ...
            pushing[soid][shard].recovery_progress.omap_recovered_to);
...
void ReplicatedBackend::do_repop(OpRequestRef op)
{
  ...
  // recovery_info.version is inconsisent of missing object need(look at on_local_recover).
  // opt is not empty in two case.
  // case1: do write before obj started to recover omap header, opt includes pglog/oi/op.
  // case2: do write after obj recovered omap header, opt includes pglog/oi/op or pglog/oi.
  if (!rm->opt.empty()) {
